...
type Payload interface {
	...
	// MarkAsProcessed is invoked by the pipeline when Payload either reaches the pipeline sink
	// or it gets discarded by one of the pipeline stages.
	MarkAsProcessed()
}

// Processor is implemented by types that can process Payloads as a part of a pipeline stage.
type Processor interface {
	// Process operates on the input payload and returns a new payload
	// to be forwarded to the next pipeline stage. Processors alo opt
	// to prevent the payload reaching the rest of the pipeline by returning
