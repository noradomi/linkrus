...
type Iterator interface {
	// Next advances the iterator. If no more items are available or
...
type EdgeIterator interface {
	...
	// Edge return the currently fetched edge objects.
	Edge() *Edge
}

// Link encapsulates all information about a link discovered by the Link 'R' Us crawler.
type Link struct {
...
type Graph interface {
	...
	// Edges returns an iterator for the set of edges whose source vertex IDs belong to the
